...
describe('Teste consultas possíveis para o usuário', () => {
  ...
  it('Cliente SIEM - CPF', () => {
    cy.get('[data-testid="inputDigiteCNPJ"]').type('634.572.703-25');
    cy.get('[data-testid="submitConsultaModulos"]').click();
    cy.url().should('equal','http://127.0.0.1:5500/docs/resultado.html?cnpj=63457270325');
  })

  it('Cliente GA - CNPJ', () => {
    cy.get('[data-testid="inputDigiteCNPJ"]').type('13.094.739/0001-51');
    cy.get('[data-testid="submitConsultaModulos"]').click();
    cy.url().should('equal','http://127.0.0.1:5500/docs/resultado.html?cnpj=13094739000151');
  });

  it('Cliente GA - CPF', () => {;
    cy.get('[data-testid="inputDigiteCNPJ"]').type('025.695.438-02');
    cy.get('[data-testid="submitConsultaModulos"]').click();
